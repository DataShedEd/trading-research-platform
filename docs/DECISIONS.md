...
DEC-028
Date: 2026-08-21
Decision: Repaired-price dataset version gbx3 (REPAIRED_SOURCE "eodhd-gbx3") supersedes gbx2, released under the holdout directive's defect protocol during the FTSE 250 extension (QNT-111/112). Generic fixes, each of which changed FTSE 100 values and therefore forced a version bump rather than an in-place edit: (1) WINDOWED MULTI-CODE ATTRIBUTION — securities carrying several provider codes over their life (Frasers SPD/FRAS, Alliance Trust ATST/ALW, F&C IT FRCL/FCIT, Royal Mail RMG/IDS, F&C CPT FCPT) previously ingested an arbitrary first-write-wins blend of overlapping payload histories; each code now contributes bars only inside its identifier validity window. (2) SINGLE-BAR VENDOR-SPIKE FILTER — a close moving >8x and reverting within two bars is a vendor artefact (EODHD's 2012-05-28 sentinel day printed literal 1,000,000.0 across several investment trusts) and is dropped with a log. (3) In-batch write dedupe in write_prices. (4) The QNT-111 FTSE 250 adjudications (price units, bar/dividend/split exclusions — all cited in unit_repair.py). Additionally, EODHD returns an identical synthetic ~100-based placeholder series (699-974 bars, 2015-2017) for symbols it does not truly carry (WTAN, TIFS, ESKN, BCPT observed); such codes are detached and the affected names join the exception list rather than carrying fabricated prices.
Context: The FTSE 250 extension's screening (density + jump scans, split-record cross-checks, coverage measurement) surfaced defects that had silently existed under the FTSE 100: the multi-code blend and the sentinel day were present in gbx2.
Alternatives: Keep gbx2 untouched and fork a 250-only dataset (two truths for shared securities — worse); patch in place (breaks the append-only versioning contract).
Reason: One dataset version with documented, generic fixes; the canonical FTSE 100 experiment re-run quantifies the effect.
Consequences: momentum-canonical re-run under gbx3 (EXP-fc09ec88, momentum-canonical-gbx3-r1): CAGR 10.69%->10.85% (+17bp), Sharpe 0.552->0.560, IR 0.267->0.280, max drawdown unchanged — the FTSE 100 SUPPORTED conclusion is unaffected in substance; prior run records are preserved and the registry manifests pin their dataset versions. Golden momentum fixtures regenerated deliberately under gbx3 (Alliance Witan's corrected series shifts one 2025 rank). The gbx2 canonical projection was rebuilt in place during the transition (its exact row set is reconstructable from the append-only raw store plus the pre-bump code in git history); canonical/derived stores are projections of raw, which remains append-only and untouched.

---

DEC-029
Date: 2026-08-21
Decision: FTSE 250 research coverage starts 2016-01-01. Measured price-coverage gaps by year (member-months without >=15 repaired bars, IPO-join months excluded): 2016 4.8%, 2017 3.5%, 2018 2.5%, 2019 2.7%, 2020 2.2%, 2021+ <=1.4%. Identity resolution is >=99% from 2013; 2013-2015 price gaps (10-14%) and pre-2013 identity gaps disqualify earlier starts under the completeness standard — the FTSE 100's 2.5% bar (DEC-016) is met from ~2018 and approached from 2017; 2016-2017's elevated gaps are accepted with prominent disclosure and a >=2018 robustness slice reported alongside any full-window result. The exception list (enumerated in tickets/QNT-112 with per-name evidence): notably Witan Investment Trust (EODHD serves a synthetic placeholder series; Tiingo has zero rows), TI Fluid Systems (same), Hastings Group Holdings (48 real EODHD bars over a 5-year membership; Tiingo has ~1 partial year), the Balanced/BMO Commercial Property Trust 2019-2024 era, Esken/Stobart post-2019, plus DEC-016-shared ex-FTSE-100 names (ICAP, AMEC, Ladbrokes, Henderson Group, African Barrick/Acacia) and shorter 2016-17 tails (Jimmy Choo, Kennedy Wilson Europe, Paysafe, SVG Capital, Betfair, Bluecrest AllBlue, Darty, Enterprise Inns, Home Retail, Poundland, Amlin). The list may only shrink; NO bias direction is claimed (DEC-025). EODHD placeholder-series detection (identical ~100-based synthetic fragments returned for symbols the vendor does not truly carry) is part of the screening toolkit.
Context: QNT-112 measurement over the QNT-111 membership; the FTSE 250 holdout must not assume the FTSE 100's coverage properties (holdout directive §5).
Alternatives: 2013 start (14% year-one gap — fails any standard); 2018 start (cleanest, but sacrifices 2 years of holdout evidence while the 2016-17 gaps are enumerated and disclosed).
Reason: Balances holdout power (10.6 years) against measured completeness, with the elevated early gaps quantified, named, and carried into the conclusion's weaknesses rather than hidden.
Consequences: The canonical FTSE 250 holdout runs 2016-01-01..2026-08-17. Coverage tables ship in the run's data-quality disclosures; the >=2018 slice is a mandatory robustness diagnostic. Tiingo free-tier cross-checks were run for headline exceptions (documented above) and do not materially remediate; QNT-105-style second-provider sourcing remains the remediation path.

---

DEC-030
Date: 2026-08-21
Decision: Post-completion review findings for the FTSE 250 holdout (QNT-115), from independent teammate research that completed after the programme:
(1) BENCHMARK FEE ASYMMETRY. MIDD's TER is 0.40%/yr vs ISF's ~0.07%: the FTSE 250 excess return is measured against a benchmark dragging ~33bp/yr more than the FTSE 100 comparison basis. Restated gross-of-benchmark-fee: FTSE 250 excess ≈ +4.23%/yr (from +4.63% measured) — the supportive-replication verdict is unaffected (the pre-registered IR band was cleared 4x), but every FTSE 100-vs-FTSE 250 excess comparison must state the basis. Secondary note: one aggregator quotes MIDD's charge at 0.52% vs the KID's 0.40% — a 12bp discrepancy to re-read at the next benchmark review.
(2) MIDD DATA COMPLETENESS VERIFIED: exactly four distributions in every full calendar year 2005-2025 (3 in the 2004 inception year) and >=252/253 sessions per year — the CUKX-style gap failure mode (DEC-021) is absent.
(3) SOURCE EDITION PINNING. FTSE Russell retroactively edits the constituent-history PDF (a 2009 row appears in today's edition but not the archived December 2019 edition). The edition built on is pinned: April 2026, sha256 028b62bc27944566... recorded alongside the parse in data_sources/ftse/. Future refreshes must diff against the pinned parse, never silently replace it.
(4) LICENCE. The PDF states reproduction/redistribution requires written LSEG permission. The raw PDF is REMOVED from the repository (kept locally and re-downloadable; the parse provenance and sha256 remain committed). The derived membership dataset stays private to this personal research repository and must not be redistributed.
(5) EXACT AD-HOC DATES. The five Apr-Aug 2026 index changes previously checkpoint-dated at 2026-08-20 now carry authoritative effective dates (Throgmorton out 04-21, Carnival out 05-06, IPF and Bluefield out 08-04, JTC out 08-20, with their cited replacements). A labelled diagnostic re-run under the corrected membership gives IR 0.447 vs the frozen canonical's 0.439 — the checkpoint-dating was mildly conservative and the frozen conclusion stands unchanged.
Context: Teammate agents (benchmark and sources research) reported after the holdout concluded; their findings independently confirm the June 2026 review reading (9-in/9-out), the MIDD/VMID selection and the constituent-history source, and add the corrections above.
Consequences: Reports comparing universes state the benchmark-fee basis; the pinned-edition rule joins the curation workflow; QNT-106 (M&A consideration) also inherits the SEDOL trail examples from the research (IPF, JTC).
