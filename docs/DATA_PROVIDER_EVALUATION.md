...
> **Owner decision gate: PASSED 2026-08-16.** The owner signed off Phase 1 by subscribing:
> EODHD paid monthly (100k requests/day observed via `/api/user`) and Tiingo Starter (free).
> Recorded as DEC-013. FMP (Phase 2) remains unpurchased pending the Phase 1 verdict below.

Status: **empirical evaluation run on live data, 2026-08-16** — see [Results](#results),
generated by the bake-off harness from real API responses, never from advertised feature
lists. Candidates evaluated live: EODHD, Tiingo. FMP is desk-research only so far.
...
`2026-08-16.2`; typed loader with strict schema validation; see the README alongside it for
...
**Live observations (2026-08-16, first real runs):** all four validation-universe LSE
delistings (Carillion, Thomas Cook, Arm, Morrisons) present in `exchange-symbol-list/LSE?delisted=1`
(4,001 rows) with ISINs matching our independently recorded expectations; price histories from
1990 for long-lived names (Rolls-Royce: 36.6 years once the LSE trailing-dot ticker quirk is
handled — EODHD codes are `RR.LSE`, not `RR..LSE`); Carillion prices continue ~4 months past the
January 2018 suspension (stale suspension-period rows to 2018-05-16 — flag such rows rather than
treating them as trading days); UK/EU fundamentals `filing_date` is ~99% equal to the period end
(a vendor default, not a filing date — unusable as `available_at`), while US filers carry
real-looking dates ~83–85% of the time. On bulk access: besides the API there is a Windows GUI
bulk downloader (eodhd.com/financial-apis/windows-downloader-end-of-day-data) — not useful for
this pipeline (manual, unscripted, outputs files outside the raw-store audit trail); the
...
<!-- BEGIN GENERATED trp.bakeoff.report (QNT-036) — DO NOT HAND-EDIT -->
...
| Field | Value |
| --- | --- |
| Run identifier | `live-2026-08-16` |
| Run started | `2026-08-16T10:53:45Z` |
| Report generated | `2026-08-16T10:53:45Z` |
| Validation universe version | `2026-08-16.2` |
| Weight-file version | `2026-08-16.1` |
| Adapter versions | `eodhd` 1.0, `tiingo` 1.0 |
| Git commit | `bb5d614cae78c37053f7d99b54e51e2addf4d557` |
| Tier tested | `eodhd`: paid monthly (100k req/day observed via /api/user), `tiingo`: Starter (free) |
| Run filters | merged_from: eodhd-final-1, eodhd-final-1b, tiingo-final-1 |
| Cells recorded | 62 |
| Declared inputs verified | not recorded |

### Summary

Totals are **ordinal summaries, not verdicts**: two providers within 0.05 of each other are not meaningfully separated, and the per-criterion breakdown below is the real output. Unmeasured criteria are excluded from the total and the remaining weights renormalised.

| Provider | Weighted total | Measured criteria | Unmeasured | Veto failures | Status |
| --- | --- | --- | --- | --- | --- |
| `eodhd` | 0.557 | 5/10 | 5 | pit_fundamentals | **UNSUITABLE (veto)** |
| `tiingo` | 0.305 | 5/10 | 5 | delisted_coverage, pit_fundamentals | **UNSUITABLE (veto)** |

Cells recorded across all providers: 62.

### eodhd

> **UNSUITABLE — veto criterion failed: pit_fundamentals = 0.125.** DEC-012 makes this decisive regardless of the weighted total: QUANT_PRINCIPLES §1 and §2 are not averageable.

Weighted total **0.557** (weights `2026-08-16.1`), from 102 check result(s) over 44 cell(s).

| Criterion | Weight | Kind | Score | Contribution | Checks | Notes |
| --- | --- | --- | --- | --- | --- | --- |
| delisted_coverage | 0.200 | empirical | 0.667 | 0.190 | 3 | — |
| pit_fundamentals | 0.200 | empirical | 0.125 | 0.036 | 40 | **VETO FAILED** |
| corporate_action_accuracy | 0.150 | empirical | 0.875 | 0.188 | 8 | — |
| historical_depth | 0.100 | empirical | 1.000 | 0.143 | 7 | — |
| identifier_stability | 0.100 | empirical | unmeasured | unmeasured | 0 | no applicable check results; excluded from the total; remaining weights renormalised |
| api_reliability | 0.050 | empirical | unmeasured | unmeasured | 0 | no applicable check results; excluded from the total; remaining weights renormalised |
| cost | 0.050 | declared | unmeasured | unmeasured | 0 | no declared input supplied; excluded from the total; remaining weights renormalised |
| licensing | 0.050 | declared | unmeasured | unmeasured | 0 | no declared input supplied; excluded from the total; remaining weights renormalised |
| rate_limits_bulk | 0.050 | declared | unmeasured | unmeasured | 0 | no declared input supplied; excluded from the total; remaining weights renormalised |
| revision_history | 0.050 | empirical | 0.000 | 0.000 | 1 | — |

#### Evidence behind each empirical score

| Criterion | Checks | pass | fail | error | n/a |
| --- | --- | --- | --- | --- | --- |
| corporate_action_accuracy | `dividend_amount_and_ex_date`, `price_continuity_across_ticker_change`, `raw_vs_adjusted_consistency`, `split_ratio_and_ex_date` | 7 | 1 | 0 | 26 |
| delisted_coverage | `delisted_price_history` | 2 | 1 | 0 | 1 |
| historical_depth | `price_history_depth` | 7 | 0 | 0 | 0 |
| pit_fundamentals | `filing_lag_distribution`, `fundamental_availability_class`, `fundamental_timestamp_plausibility`, `fundamental_timestamp_presence` | 5 | 35 | 0 | 16 |
| revision_history | `restatement_visibility` | 0 | 1 | 0 | 0 |

#### Example failed checks

38 failing or errored check result(s); the 3 on the heaviest criteria are shown (tie-break: criterion, security, check name, explanation). Every result, passing or failing, is in the run's `cells.jsonl` with the same evidence fields.

- **delisted_coverage** (weight 0.200) — `delisted_price_history` on `carillion` / prices → **fail**
  - expected: daily prices up to 2018-01-15 (failure)
  - observed: final row 2018-05-16 close=14.27, 4563 rows
  - reading: prices continue 121 days past the delisting date: the security did not trade then, so these rows are fabricated or belong to a reused ticker — worse than missing data because they look real
  - raw payload: `data/raw/eodhd/prices/18609b41161f4c9d/20260816T103406.403564Z-0.meta.json`
- **pit_fundamentals** (weight 0.200) — `fundamental_availability_class` on `apple` / fundamentals → **fail**
  - expected: availability class first_known or filing_only (a defensible available_at per QNT-020)
  - observed: period_end_only [us] from field(s) filing_date over 594 statement(s)
  - reading: period ends only: every available_at would be a DEC-007 imputation, flagged as such, and point-in-time claims on this data are bounded by that assumption
  - raw payload: `data/raw/eodhd/fundamentals/6ada899ebad6daf2/20260816T103402.516157Z-0.meta.json`
- **pit_fundamentals** (weight 0.200) — `fundamental_timestamp_presence` on `apple` / fundamentals → **fail**
  - expected: >= 90% of statements carry a usable publication timestamp (after period end, not future, not a placeholder)
  - observed: 486/594 (82%) via field(s) filing_date [us]
  - reading: too few statements carry a usable publication timestamp: availability for the remainder must be imputed conservatively (DEC-007) and flagged, which bounds what any factor built on them can claim
  - raw payload: `data/raw/eodhd/fundamentals/6ada899ebad6daf2/20260816T103402.516157Z-0.meta.json`

#### Measurements

Observations recorded by the checks rather than judgements of the provider; they score nothing (see `trp.bakeoff.payloads.MEASUREMENT_PREFIX`). Each is one line of observed values; a measurement that contradicts a recorded decision is shown in full.

**4 measurement(s) contradict a recorded decision** and are quoted in full below; act on them by superseding the decision, never by editing it.

- `filing_lag_distribution` on `apple`: annual: n=96, median=34d, p75=75d, p90=82d, max=90d; quarterly: n=390, median=32d, p75=41d, p90=62d, max=182d
  - MEASUREMENT: filing-lag distribution for apple (us). DECISION-TRIGGER: annual: observed p90 82d EXCEEDS DEC-007's assumed 60d — the assumption is not conservative enough and imputed availability would be earlier than reality for the tail. Supersede DEC-007 with a new decision entry; do not edit DEC-007 in place. DECISION-TRIGGER: quarterly: observed p90 62d EXCEEDS DEC-007's assumed 45d — the assumption is not conservative enough and imputed availability would be earlier than reality for the tail. Supersede DEC-007 with a new decision entry; do not edit DEC-007 in place.
- `filing_lag_distribution` on `arm-holdings`: 0 of 0 statement(s) carry a usable timestamp
- `filing_lag_distribution` on `astrazeneca`: quarterly: n=3, median=37d, p75=37d, p90=37d, max=37d
- `filing_lag_distribution` on `carillion`: 0 of 187 statement(s) carry a usable timestamp
- `filing_lag_distribution` on `citigroup`: annual: n=99, median=58d, p75=67d, p90=85d, max=90d; quarterly: n=393, median=38d, p75=51d, p90=59d, max=90d
  - MEASUREMENT: filing-lag distribution for citigroup (us). DECISION-TRIGGER: annual: observed p90 85d EXCEEDS DEC-007's assumed 60d — the assumption is not conservative enough and imputed availability would be earlier than reality for the tail. Supersede DEC-007 with a new decision entry; do not edit DEC-007 in place. DECISION-TRIGGER: quarterly: observed p90 59d EXCEEDS DEC-007's assumed 45d — the assumption is not conservative enough and imputed availability would be earlier than reality for the tail. Supersede DEC-007 with a new decision entry; do not edit DEC-007 in place.
- `filing_lag_distribution` on `flutter`: annual: n=8, median=63d, p75=86d, p90=86d, max=86d; quarterly: n=25, median=44d, p75=58d, p90=86d, max=86d
  - MEASUREMENT: filing-lag distribution for flutter (uk). annual: observed p90 86d is within DEC-007's assumed 90d, so the imputation is conservative for this sample. DECISION-TRIGGER: quarterly: observed p90 86d EXCEEDS DEC-007's assumed 60d — the assumption is not conservative enough and imputed availability would be earlier than reality for the tail. Supersede DEC-007 with a new decision entry; do not edit DEC-007 in place.
- `filing_lag_distribution` on `microsoft`: annual: n=99, median=32d, p75=68d, p90=89d, max=91d; quarterly: n=393, median=29d, p75=43d, p90=57d, max=91d
  - MEASUREMENT: filing-lag distribution for microsoft (us). DECISION-TRIGGER: annual: observed p90 89d EXCEEDS DEC-007's assumed 60d — the assumption is not conservative enough and imputed availability would be earlier than reality for the tail. Supersede DEC-007 with a new decision entry; do not edit DEC-007 in place. DECISION-TRIGGER: quarterly: observed p90 57d EXCEEDS DEC-007's assumed 45d — the assumption is not conservative enough and imputed availability would be earlier than reality for the tail. Supersede DEC-007 with a new decision entry; do not edit DEC-007 in place.
- `filing_lag_distribution` on `morrisons`: 0 of 665 statement(s) carry a usable timestamp
- `filing_lag_distribution` on `rolls-royce`: quarterly: n=1, median=1593d, p75=1593d, p90=1593d, max=1593d
- `filing_lag_distribution` on `sap`: quarterly: n=3, median=24d, p75=24d, p90=24d, max=24d
- `filing_lag_distribution` on `shell`: quarterly: n=3, median=30d, p75=30d, p90=30d, max=30d
- `filing_lag_distribution` on `tesco`: annual: n=29, median=1d, p75=1d, p90=1d, max=1d; quarterly: n=59, median=4d, p75=8d, p90=30d, max=30d
- `filing_lag_distribution` on `thomas-cook`: 0 of 184 statement(s) carry a usable timestamp
- `filing_lag_distribution` on `unilever`: quarterly: n=2, median=1d, p75=1d, p90=1d, max=1d

#### What was not measured

| Fetch status | Cells | Datasets affected |
| --- | --- | --- |
| ok | 44 | corporate_actions, delisted_securities, fundamentals, prices, securities |

Throttling events during the run: 0. Checks that errored: 0.

### tiingo

> **UNSUITABLE — veto criterion failed: delisted_coverage = 0.000, pit_fundamentals = 0.000.** DEC-012 makes this decisive regardless of the weighted total: QUANT_PRINCIPLES §1 and §2 are not averageable.

Weighted total **0.305** (weights `2026-08-16.1`), from 18 check result(s) over 18 cell(s).

| Criterion | Weight | Kind | Score | Contribution | Checks | Notes |
| --- | --- | --- | --- | --- | --- | --- |
| delisted_coverage | 0.200 | empirical | 0.000 | 0.000 | 0 | **VETO FAILED**; dataset(s) not offered by provider/tier: delisted_securities |
| pit_fundamentals | 0.200 | empirical | 0.000 | 0.000 | 4 | **VETO FAILED** |
| corporate_action_accuracy | 0.150 | empirical | 0.857 | 0.171 | 7 | — |
| historical_depth | 0.100 | empirical | 1.000 | 0.133 | 1 | — |
| identifier_stability | 0.100 | empirical | 0.000 | 0.000 | 0 | dataset(s) not offered by provider/tier: securities |
| api_reliability | 0.050 | empirical | unmeasured | unmeasured | 0 | no applicable check results; excluded from the total; remaining weights renormalised |
| cost | 0.050 | declared | unmeasured | unmeasured | 0 | no declared input supplied; excluded from the total; remaining weights renormalised |
| licensing | 0.050 | declared | unmeasured | unmeasured | 0 | no declared input supplied; excluded from the total; remaining weights renormalised |
| rate_limits_bulk | 0.050 | declared | unmeasured | unmeasured | 0 | no declared input supplied; excluded from the total; remaining weights renormalised |
| revision_history | 0.050 | empirical | unmeasured | unmeasured | 0 | no applicable check results; excluded from the total; remaining weights renormalised |

#### Evidence behind each empirical score

| Criterion | Checks | pass | fail | error | n/a |
| --- | --- | --- | --- | --- | --- |
| corporate_action_accuracy | `dividend_amount_and_ex_date`, `raw_vs_adjusted_consistency`, `split_ratio_and_ex_date` | 6 | 1 | 0 | 2 |
| historical_depth | `price_history_depth` | 1 | 0 | 0 | 0 |
| pit_fundamentals | `filing_lag_distribution`, `fundamental_availability_class`, `fundamental_timestamp_plausibility`, `fundamental_timestamp_presence` | 0 | 4 | 0 | 4 |

#### Example failed checks

5 failing or errored check result(s); the 3 on the heaviest criteria are shown (tie-break: criterion, security, check name, explanation). Every result, passing or failing, is in the run's `cells.jsonl` with the same evidence fields.

- **pit_fundamentals** (weight 0.200) — `fundamental_availability_class` on `apple` / fundamentals → **fail**
  - expected: availability class first_known or filing_only (a defensible available_at per QNT-020)
  - observed: period_end_only [us] from field(s) none over 15 statement(s)
  - reading: period ends only: every available_at would be a DEC-007 imputation, flagged as such, and point-in-time claims on this data are bounded by that assumption
  - raw payload: `data/raw/tiingo/fundamentals/6ada899ebad6daf2/20260816T103453.845776Z-0.meta.json`
- **pit_fundamentals** (weight 0.200) — `fundamental_timestamp_presence` on `apple` / fundamentals → **fail**
  - expected: >= 90% of statements carry a usable publication timestamp (after period end, not future, not a placeholder)
  - observed: 0/15 (0%) via field(s) none [us]
  - reading: too few statements carry a usable publication timestamp: availability for the remainder must be imputed conservatively (DEC-007) and flagged, which bounds what any factor built on them can claim
  - raw payload: `data/raw/tiingo/fundamentals/6ada899ebad6daf2/20260816T103453.845776Z-0.meta.json`
- **pit_fundamentals** (weight 0.200) — `fundamental_availability_class` on `microsoft` / fundamentals → **fail**
  - expected: availability class first_known or filing_only (a defensible available_at per QNT-020)
  - observed: period_end_only [us] from field(s) none over 15 statement(s)
  - reading: period ends only: every available_at would be a DEC-007 imputation, flagged as such, and point-in-time claims on this data are bounded by that assumption
  - raw payload: `data/raw/tiingo/fundamentals/c8639a557172457e/20260816T103512.868808Z-0.meta.json`

#### Measurements

Observations recorded by the checks rather than judgements of the provider; they score nothing (see `trp.bakeoff.payloads.MEASUREMENT_PREFIX`). Each is one line of observed values; a measurement that contradicts a recorded decision is shown in full.

- `filing_lag_distribution` on `apple`: 0 of 15 statement(s) carry a usable timestamp
- `filing_lag_distribution` on `microsoft`: 0 of 15 statement(s) carry a usable timestamp

#### What was not measured

| Fetch status | Cells | Datasets affected |
| --- | --- | --- |
| ok | 8 | corporate_actions, fundamentals, prices |
| unsupported | 9 | delisted_securities, financial_periods, securities |
| provider_error | 1 | fundamentals |

Throttling events during the run: 0. Checks that errored: 0.
An `unsupported` dataset is a capability gap, scored zero on the criteria that depend on it — distinct from a criterion nobody measured, which is excluded.

### Recommendation

> **OWNER DECISION GATE — nothing is purchased without the owner's sign-off.** This is a recommendation generated from the scores above, not a decision. Re-verify every price on the day of purchase and record the choice in `DECISIONS.md`.

**No candidate is recommendable from this run.** Every provider either failed a veto criterion or produced no measurable results, so there is nothing to choose between. Fix the run or widen the field before deciding anything.

<!-- END GENERATED trp.bakeoff.report -->
